# webcam threading: https://gist.github.com/docPhil99/ca4da12c9d6f29b9cea137b617c7b8b1
...
# Conversion to .exe: get absolute path to resource, works for dev and for PyInstaller
...
class VideoThread(QThread):
   ...
   def run(self): # capture from webcam
      ...
      # ['helm', 'hoed', 'hoofd'], hoed wordt ook beschouwd als hoofd (= niet helm)
      class_names = ['helm', 'hoofd', 'hoofd']

      if self.t:  # parameters van yolov4-tiny toepassen
         shiftregister_length, shiftregister_threshold, max_counter = 10, 7, 100
         weightsPath, configPath = resource_path('yolov4-tiny_training_best.weights'), \
                                   resource_path('yolov4-tiny_testing.cfg')
      else:  # parameters van gewone yolov4 toepassen
         shiftregister_length, shiftregister_threshold, max_counter = 7, 5, 10
         weightsPath, configPath = resource_path('yolov4_training_best.weights'), \
                                   resource_path('yolov4_testing.cfg')
      ...
      while self._run_flag:
         ...
         for (classid, score, box) in zip(classes, scores, boxes):
            ...
            # achtergrondkleur voor benaming
            cv2.rectangle(frame, (box[0], box[1]), (box[0] + 100, box[1] + 15), color, cv2.FILLED)
            # benaming van de bounding box
            cv2.putText(frame, label, (box[0] + 1, box[1] + 13), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)
         ...
         # poort openen als de threshold bereikt is
         if shiftregister.count(1) >= shiftregister_threshold and openpoort is False:
            print("OKEEE\nPOORT OPENT")
            openpoort = True
...
class App(QWidget):
   def __init__(self):
      ...
      self.setGeometry(400, 150, 750, 620)  # positie van window, width + height
      ...
      self.widgets = {"logo": [],
                      ...
                      "close_button": [],
                      "copyright_label": []}
      self.setWindowIcon(QIcon(resource_path('helmicon.png')))
      self.startframe()
   ...
   def startframe(self):
      ...
      # create a logo label
      self.image = QPixmap(resource_path("logokuleuven.png"))
      ...
      self.copyright_text = QLabel("\u00A9 Lowie Deboel, Vincent Renders")
      self.copyright_text.setStyleSheet("font-size: 10px;")
      self.copyright_text.setAlignment(Qt.AlignRight)
      self.widgets["copyright_label"].append(self.copyright_text)
      ...
      self.grid.addWidget(self.copyright_text, 7, 0, 1, 3)

   # keuze van poort bij combobox doorgeven aan detectiemodel
   ...
   def webcamframe(self):
      ...
      # widgets op grid plaatsen
      self.grid.addWidget(self.webcam_label, 0, 0, 43, 3)
      self.grid.addWidget(self.exit_button, 43, 0, 1, 3)
      self.grid.addWidget(self.copyright_text, 44, 0, 1, 3)
